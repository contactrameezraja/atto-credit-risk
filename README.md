...
| Renamed fields to match brief | Brief shows num_transactions, has_rent etc. | API contract matches the specification |
...
- `POST /predict` accepts customer features and returns a default probability and binary prediction
...
- Added Pydantic `Field` constraints (e.g. `has_rent` only accepts 0 or 1, `total_credit` must be >= 0) so invalid data is rejected before it reaches the model
