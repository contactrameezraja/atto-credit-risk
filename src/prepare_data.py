...
if __name__ == "__main__":
    ...
    # Data quality report: the brief asks us to explore and document issues.
    # This summary runs on every execution so problems are caught early.
    print("\n========== DATA QUALITY REPORT ==========")
    ...
    n_bad_amounts = tx["amount"].isna().sum()
    if n_bad_amounts > 0:
    ...
    n_dupes = tx["transaction_id"].duplicated(keep="first").sum()
    if n_dupes > 0:
        print(f"WARNING: {n_dupes} duplicate transaction_id(s) found - removing them")
        tx = tx.drop_duplicates(subset=["transaction_id"], keep="first")
    else:
    ...
    iqr = q3 - q1
    outlier_mask = (tx["amount"] < q1 - 1.5 * iqr) | (tx["amount"] > q3 + 1.5 * iqr)
    ...
    # These 5 columns are required by the pre-trained model, so we keep them as-is.
    ...
    # captures broader spending patterns. For example, "tesco" alone misses
    # someone who shops at "asda", but a "grocery" category catches both.
    MERCHANT_CATEGORIES = {
    ...
    # It was only needed for keyword/category matching above. It's not a
    # model feature and shouldn't be in the final training set.
    ...
    # The labels file uses "defaulted_within_90d" but the brief shows "defaulted".
    df = df.rename(columns={"defaulted_within_90d": "defaulted"})

    # Rename columns to match the brief's expected format
    # The brief shows "num_transactions" not "txn_count".
    ...
    # we would drop kw_ and use the has_ category columns instead, since they
    # capture broader merchant groups rather than individual keywords.
